import React from 'react';
import Card from 'react-bootstrap/Card';
import { Button } from 'antd';
import './displayQuizCard.css';
import { useState } from 'react';
import { CommentArea } from './CommentArea';
import { DisplayQuizOptions } from './DisplayQuizOptions';
import {ReactComponent as QuizIcon} from '../../Quiz.svg';


export interface DisplayQuizCardProps{

    card:contentCard;
    updateContentCardControl: (lessonId: number,cardId:number,field:string,value:any) => void


}

export const DisplayQuizCard: React.FC<DisplayQuizCardProps> = ({card,updateContentCardControl}) =>{

    const[comment,setComment]=useState(false);

    function openComment(){
        setComment(comment=>!comment);
    }
    return(
    <div className="DisplayQuizCard">
        {
        comment &&
        <CommentArea card={card} updateContentCardControl={updateContentCardControl}/>
        }
        <Card className="ImageUpload Display">
            <Card.Header className="FooterButtons">
                <Button disabled size="large"
                    style={{backgroundColor:"white", borderRadius:"15px 0px 0px 0px",marginRight:"5px"}}>
                    <QuizIcon /></Button>Quiz
                <Button className="DisplayPoints" size="small"
                    style={{backgroundColor:"#EF6315", borderRadius:"5px"}}>{card.points} Points</Button>
                <Button className="DisplayComments" size="large" onClick={openComment}>
                    <img alt="" className="DisplayCommentIcon"
                        src="data:image/svg+xml;base64," />
                { card.comment &&
                    <div className="isComment">*</div>
                    }
                </Button>
            </Card.Header>
            <div className="DisplayQuizQuestion">
                {card.content.QuizQuestion}
            </div>
            <div className="QuizOptions">
                {card.content.options?.map((option)=>(<DisplayQuizOptions option={option}/>))}
            </div>
            <p style={{fontSize:"10px",paddingLeft:"5px"}}>Correct Answer explanation</p>
            <div className="DisplayQuizExplanation">
                {card.content.QuizExplanation}
            </div>
        </Card>
    </div>
    );
};