import React from 'react';
import Card from 'react-bootstrap/Card';
import { Button } from 'antd';
import './displayFileCard.css';
import { useState } from 'react';
import { CommentArea } from './CommentArea';
import {ReactComponent as PDFIcon} from '../../PDF.svg';


export interface DisplayFileCardProps{

    card:contentCard;
    updateContentCardControl: (lessonId: number,cardId:number,field:string,value:any) => void


}

export const DisplayFileCard: React.FC<DisplayFileCardProps> = ({card,updateContentCardControl}) =>{

    const[comment,setComment]=useState(false);

    function openComment(){
        setComment(comment=>!comment);
    }
    return(
    <div className="DisplayFileCard">
        {
        comment &&
        <CommentArea card={card} updateContentCardControl={updateContentCardControl}/>
        }
        <Card className="ImageUpload Display">
            <Card.Header className="FooterButtons">
                <Button disabled size="large"
                    style={{backgroundColor:"white", borderRadius:"15px 0px 0px 0px",marginRight:"5px"}}>
                    <PDFIcon /></Button>PDF
                <Button className="DisplayPoints" size="small"
                    style={{backgroundColor:"#EF6315", borderRadius:"5px"}}>{card.points} Points</Button>
                <Button className="DisplayComments" size="large" onClick={openComment}>
                <img alt="" className="DisplayCommentIcon"
                        src="data:image/svg+xml;base64," />
                { card.comment &&
                    <div className="isComment">*</div>
                    }
                </Button>
            </Card.Header>
            <div className="DisplayPDFTitle">{card.content.PDFTitle}</div>
                <div className="DisplayFile">
                    <p className="DisplayFileName"> {card.content.PDFName}</p> 
                    <div className="DisplayFileSize">PDF size in KBs {card.content.PDFSize}</div>
                    <Button className="DisplayFileView" type="primary" shape="round" size="small" href={card.content.PDFFile} target="_blank">Open</Button>
                </div>
            <Card.Body className="DisplayFileDescription">
                <p style={{fontSize:"10px"}}>Description</p>
                {card.content.PDFDescription}
            </Card.Body>
        </Card>
    </div>
    );
    };