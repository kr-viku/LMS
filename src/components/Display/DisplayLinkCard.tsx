import React from 'react';
import Card from 'react-bootstrap/Card';
import { Button } from 'antd';
import './displayLinkCard.css';
import { LinkOutlined } from '@ant-design/icons';
import { useState } from 'react';
import { CommentArea } from './CommentArea';
import {ReactComponent as LinkIcon} from '../../Link.svg';


export interface DisplayLinkCardProps{

    card:contentCard;
    updateContentCardControl: (lessonId: number,cardId:number,field:string,value:any) => void


}

export const DisplayLinkCard: React.FC<DisplayLinkCardProps> = ({card,updateContentCardControl}) =>{

    const[comment,setComment]=useState(false);

    function openComment(){
        setComment(comment=>!comment);
    }
    return(
    <div className="DisplayLinkCard">
         {
        comment &&
        <CommentArea card={card} updateContentCardControl={updateContentCardControl}/>
        }
        <Card className="ImageUpload Display">
            <Card.Header className="FooterButtons">
                <Button disabled size="large"
                    style={{backgroundColor:"white", borderRadius:"15px 0px 0px 0px",marginRight:"5px"}}>
                    <LinkIcon/></Button>External Link
                <Button className="DisplayPoints" size="small"
                    style={{backgroundColor:"#EF6315", borderRadius:"5px"}}>{card.points} Points</Button>
                <Button className="DisplayComments" size="large" onClick={openComment}>
                    <img  className="DisplayCommentIcon" alt=""
                        src="data:image/svg+xml;base64," />
                { card.comment &&
                    <div className="isComment">*</div>
                    }
                </Button>
            </Card.Header>
            <Card.Img className="DisplayImage"
                src={card.content.LinkImageFile} />
            <div className="DisplayPDFTitle">{card.content.LinkTitle}</div>
            <Card.Body className="DisplayFileDescription">
                <p style={{fontSize:"10px"}}>Description</p>
                {card.content.LinkDescription}
            
            <div className="DisplayFile">
                <p className="DisplayFileName">{card.content.Link}</p>
                <Button className="DisplayFileView" type="primary" shape="round" size="small" href={card.content.Link}
                    target="_blank">Open</Button>
            </div>
            </Card.Body>
        </Card>
    </div>
    );
};